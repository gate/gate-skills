...
| [gate-exchange-earn](#-gate-exchange-earn) | Gate 智能理财（L2，gate-cli）：活期/定期、双币、质押、收益对比与闲置方案；申购/赎回需 Action Draft + 确认 | `2026.4.29-3` | ✅ Active |
...
| [gate-exchange-newcoin](#-gate-exchange-newcoin) | 新币尽调与事件雷达（gate-cli）：上架、基本面、风险、情绪、盘口、LaunchPool 日历；可选首笔现货/Alpha（需草案确认） | `2026.4.29-1` | ✅ Active |
...
> **路径**: `skills/gate-exchange-earn/`

Gate 智能理财（L2），通过 `gate-cli` 覆盖 Simple Earn（活期/定期）、双币理财、质押、收益对比、闲置资金方案及持仓/历史。申购、赎回、双币下单、质押兑换、活期续投等写操作仅在 **Action Draft** 与用户明确 **Y** 确认后执行。需安装 `gate-cli` 并配置 Gate API。

**示例提示词**：
- `有哪些 USDT 定期理财？按年化从高到低排`
- `对比一下 ETH 的双币理财和链上质押哪个更合适`
- `最近三个月活期 Simple Earn 利息分别赚了多少`
- `申购 500 USDT 到 30 天定期`（先草案再确认）
...
> **路径**: `skills/gate-exchange-newcoin/`

新币尽调与事件雷达：通过 `gate-cli` 串联 info / news / cex，覆盖上架公告、基本面、风险、情绪、盘口、LaunchPool 日历等；可选首笔现货或 Alpha 下单，仍需 **Action Draft** 与 **Y** 确认。需 `gate-cli` 与 Gate API。

**示例提示词**：
- `这周上新币做个尽调`
- `LaunchPool 日历和近期上线`
- `这个新币会不会 Rug？查风险和社区情绪`
- `上新后小额首买 SYMBOL`（先草案再确认）
