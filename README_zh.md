...
| Skill | 描述 | 版本 | 状态 |
|-------|------|------|------|
| [gate-exchange-trading-copilot](#-gate-exchange-trading-copilot) | 端到端交易副驾驶：行情研判、风控、订单草稿、执行、交易后管理 | `2026.3.14-3` | ✅ Active |
...
| [gate-exchange-crossex](#-gate-exchange-crossex) | Gate CrossEx 跨所操作：跨 Gate、Binance、OKX、Bybit 的订单查询、持仓查询与历史查询 | `2026.3.12-1` | ✅ Active |
| [gate-exchange-spot](#-gate-exchange-spot) | Gate 现货交易：买卖下单、订单管理、账户查询、资产兑换 | `2026.3.12-1` | ✅ Active |
...
| [gate-exchange-flashswap](#-gate-exchange-flashswap) | Gate 闪兑查询：支持币对、兑换限额、订单历史与订单追踪 | `2026.3.11-5` | ✅ Active |
...
| [gate-dex-market](#-gate-dex-market) | Gate DEX 行情数据：MCP + OpenAPI 双模式智能路由，代币信息、K线、排行、安全审计 | `2026.3.14-2` | ✅ Active |
| [gate-dex-trade](#-gate-dex-trade) | Gate DEX 交易：MCP + OpenAPI 双模式智能路由，Swap 执行，跨链支持 | `2026.3.14-2` | ✅ Active |
| [gate-dex-wallet](#-gate-dex-wallet) | Gate DEX 综合钱包：身份认证、资产查询、转账执行、DApp 交互、CLI 命令行 — 5 大模块与多技能路由 | `2026.3.14-2` | ✅ Active |
...
Gate DEX 综合行情数据 Skill，支持 MCP + OpenAPI 双模式智能路由。作为行情数据查询的统一管理与分发中心，提供代币信息、K线数据、排行榜、安全审计等只读查询，并参与 Market/Trade/Wallet 多技能优先级路由系统。
...
Gate DEX 交易综合 Skill，支持 MCP + OpenAPI 双模式智能路由。纯路由层，自动检测环境并分发到完整子规范。支持跨链与单链 Swap 执行，参与 Market/Trade/Wallet 多技能优先级路由系统。
...
Gate DEX 综合钱包 Skill。统一入口，涵盖身份认证、资产查询、转账执行、DApp 交互、CLI 命令行五大模块，支持子功能路由分发，参与 Market/Trade/Wallet 多技能优先级路由系统。
...
├── image/                              # 安装截图
└── skills/
    ├── gate-dex-market/                # DEX 行情数据 skill（MCP + OpenAPI 双模式）
    ├── gate-dex-trade/                 # DEX 交易 skill（MCP + OpenAPI 双模式，跨链）
    ├── gate-dex-wallet/                # DEX 综合钱包 skill（5 大模块 + CLI）
