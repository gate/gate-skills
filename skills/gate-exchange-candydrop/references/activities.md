...
- **Tool calls**: When passing **`rule_name`** as a query parameter to `cex_launch_get_candy_drop_activity_list_v4`, keep using the API filter token (e.g. `futures`, `simple_earn`) — only **end-user visible** text is translated.
...
2. **Call tool**: Call `cex_launch_get_candy_drop_activity_list_v4` with optional filters. Use `limit=10`, `offset=0` by default.
...
2. **Call tool**: Call `cex_launch_get_candy_drop_activity_rules_v4` with `currency` and/or `activity_id`.
...
2. Call `cex_launch_get_candy_drop_activity_list_v4` with `status={value}`, `limit=10`, `offset=0` (use `limit=30` if the user clearly wants “all” and status is narrow, within API max).
...
2. Call `cex_launch_get_candy_drop_activity_list_v4` with `currency={token}`, `limit=10`, `offset=0`.
...
2. Call `cex_launch_get_candy_drop_activity_list_v4` with `rule_name={value}`, `limit=10`, `offset=0`.
3. For each activity display all fields.
...
2. Call `cex_launch_get_candy_drop_activity_list_v4` with `register_status={value}`, `limit=10`, `offset=0`.
...
2. Call `cex_launch_get_candy_drop_activity_rules_v4` with the provided parameter(s).
...
**Expected Behavior**:
1. Call `cex_launch_get_candy_drop_activity_list_v4` with user's filters.
