...
|-----------|----------|-------------|
| List open orders | `gate-cli cex futures price-trigger list` | "Show my TP/SL orders", "List conditional orders" |
| Get order detail | `gate-cli cex futures price-trigger get` | "Show details of order #123" |
| Cancel single | `gate-cli cex futures price-trigger cancel` | "Cancel that stop loss", "Cancel order #456" |
| Cancel all | `gate-cli cex futures price-trigger cancel-all` | "Cancel all TP/SL for BTC", "Cancel all triggered orders" |
| Amend order | `gate-cli cex futures price-trigger update` | "Change TP to 73000", "Update trigger price", "Amend size" |
...
After cancel or amend, call `gate-cli cex futures price-trigger get` or `gate-cli cex futures price-trigger list` to confirm the result.
...
1. Call `gate-cli cex futures price-trigger list`.
...
1. **Locate order**: if user provides an order ID, use it directly. If not, call `gate-cli cex futures price-trigger list` to show open orders and let user identify the one to cancel.
2. **Confirm**: ask *"Cancel price-triggered order #[id] for [contract] (trigger [rule] [price])? Reply to confirm."*
3. **Cancel**: call `gate-cli cex futures price-trigger cancel`.
4. **Verify**: call `gate-cli cex futures price-trigger get` → confirm status is `cancelled`.
...
3. **Cancel all**: call `gate-cli cex futures price-trigger cancel-all`.
4. **Verify**: call `gate-cli cex futures price-trigger list` → confirm list is empty (or reduced).
...
2. **Get current values**: call `gate-cli cex futures price-trigger get` to show what will change.
3. **Determine position mode**: call `gate-cli cex futures account get` to get `position_mode` (single/dual). This determines the correct `close` and `auto_size` values to include in the amend request.
...
7. **Amend**: call `gate-cli cex futures price-trigger update` with changed fields **plus** the correct `close`/`auto_size` for the current mode.
8. **Verify**: call `gate-cli cex futures price-trigger get` to confirm new values.
...
5. Call `gate-cli cex futures price-trigger cancel`.
6. Verify via `gate-cli cex futures price-trigger get` → status `cancelled`.
...
**Expected Behavior**:
1. Call `gate-cli cex futures price-trigger list` to find the TP order (trigger `>=` for long TP).
2. Get position mode via `gate-cli cex futures account get` to determine `close`/`auto_size` for amend.
...
5. Call `gate-cli cex futures price-trigger update` with `trigger_price="75000"`, `price` (current execution price), and mode-appropriate flags:
   - Single mode full close: `close = true`, no `auto_size`.
   - Dual mode full close: `auto_size = "close_long"`, no `close = true`.
6. Verify via `gate-cli cex futures price-trigger get` → confirm new trigger price.
...
2. Call `gate-cli cex futures price-trigger cancel-all` (no contract filter).
3. Verify via `gate-cli cex futures price-trigger list` → confirm list is empty.
...
**Expected Behavior**:
1. Call `gate-cli cex futures price-trigger list` to find the SL order.
2. Get position mode via `gate-cli cex futures account get` to determine `close`/`auto_size` for amend.
...
5. Call `gate-cli cex futures price-trigger update` with `price="57500"`, `trigger_price` (current trigger), and mode-appropriate flags:
   - Single mode full close: `close = true`, no `auto_size`.
   - Dual mode full close: `auto_size = "close_short"`, no `close = true`.
6. Verify via `gate-cli cex futures price-trigger get`.
