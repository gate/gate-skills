...
2. Call `gate-cli cex sub-account get`
...
**Expected Behavior**:
1. Call `gate-cli cex sub-account list` to retrieve all normal sub-accounts
...
**Expected Behavior**:
1. Call `gate-cli cex sub-account list` to check the current number of normal sub-accounts and confirm creation availability
...
5. On confirmation: call `gate-cli cex sub-account create`
...
**Expected Behavior**:
1. Extract `user_id` from user input (e.g. `123456`)
2. Call `gate-cli cex sub-account get` to verify the sub-account exists and belongs to the main account
...
5. On confirmation: call `gate-cli cex sub-account lock`
...
**Expected Behavior**:
1. Extract `user_id` from user input (e.g. `123456`)
2. Call `gate-cli cex sub-account get` to verify the sub-account exists and is currently locked
...
5. On confirmation: call `gate-cli cex sub-account unlock`
...
3. Optionally, call `gate-cli cex sub-account list` and display available sub-accounts for the user to choose from
...
**Expected Behavior**:
1. Call `gate-cli cex sub-account get`
...
2. Call `gate-cli cex sub-account create`
