...
4. Fetch executable spot context via `gate-cli cex spot market pair` and `gate-cli cex spot market tickers`
...
**Expected Behavior**:
1. Fetch data via `gate-cli cex futures market contract` and `gate-cli cex futures market tickers`
2. Fetch microstructure via `gate-cli cex futures market orderbook`, `gate-cli cex futures market liquidations`, and `gate-cli cex futures market funding-rate`
...
3. Fetch market confirmation via `info_markettrend_get_kline(symbol="ETH", timeframe="15m", period="24h", with_indicators=true)` and `gate-cli cex spot market tickers`
...
**Expected Behavior**:
1. Fetch data via `gate-cli cex spot order all-open`
...
4. Fetch execution via `gate-cli cex spot order amend` after confirmation
...
**Expected Behavior**:
1. Fetch data via `gate-cli cex futures position list`
2. Calculate side, size, and the requested reduce-only amount
3. Output a close draft if confirmation is still needed
4. Fetch execution via `gate-cli cex futures order add; gate-cli cex futures order close; gate-cli cex futures order long; gate-cli cex futures order remove; gate-cli cex futures order short` with reduce-only intent
