...
## Scenario 13: Aggregated Summary Query

**Context**: User wants server-side totals (rebate, volume, net fee, customers, trading users when applicable) via the aggregated partner endpoint, without listing every trade or commission row.

**Prompt Examples**:
- "Query my rebate"
- "How much rebate did I earn this week?"
- "Give me an aggregated summary of my partner data"
- "Total overview of my affiliate stats for last month"
- "Overall statistics for my team (all business types)"
- "Spot-only aggregated rebate and volume this week"

**Expected Behavior**:
1. Parse intent as `aggregated_summary` and optional `business_type` (default all = 0)
2. Map user time range to `start_date` / `end_date` in UTC+8 `yyyy-mm-dd hh:ii:ss`, or omit for default last 7 days; **maximum 180 days inclusive per single aggregated request**
3. Call MCP `cex_rebate_get_partner_agent_data_aggregated` or `GET /rebate/partner/data/aggregated` **once** for the full window when the range is **≤180 days** — **do not** split into multiple aggregated calls for the same user request
4. Return `rebate_amount`, `trade_volume`, `net_fee`, `customer_count`, and `trading_user_count` only when `business_type=0` (otherwise null)
5. Include `time_range_desc` and `business_type_desc` from the response in the reply

## Scenario 14: Commission / Rebate Records (Line Items)

**Context**: User wants per-entry commission or rebate history (list, ledger, records), not a single total from the aggregated endpoint.

**Prompt Examples**:
- "Show my commission records"
- "Rebate history for the last 7 days"
- "List all commissions this month"
- "Commission ledger detail"

**Expected Behavior**:
1. Parse intent as `commission_records` (not `aggregated_summary`)
2. Convert time range to Unix `from` / `to` (UTC+8 day boundaries); split into ≤30-day segments if needed (same rules as other history APIs)
3. Call MCP `cex_rebate_partner_commissions_history` or `GET /rebate/partner/commission_history`; paginate until all pages are retrieved when appropriate
4. Do not pass `user_id` unless the user names a specific trader UID
5. **Never** issue one `commission_history` request spanning **>30 days** (e.g. a single 90-day `from`/`to` window is invalid); always use segmented windows
6. Present rows (time, asset, amount, trader fields as returned) or a concise summary of the list