# Gate Exchange Dual Investment — Scenarios & Prompt Examples (13 Cases)
...
3. Display as comparison table. All timestamps formatted as `yyyy-MM-dd` in UTC+0. APY display as `apy_display × 100`%
...
3. Display: lock period (`invest_days` — integer, no decimals), delivery date **(UTC)**, target prices available, APY (`apy_display × 100`%)
4. All timestamps (e.g. `delivery_timest`) formatted as `yyyy-MM-dd` in UTC+0
...
**Expected Behavior**:
1. Call `cex_earn_list_dual_investment_plans()` to get plan details (`apy_display`, target price, delivery period)
2. Display APY as `apy_display × 100`%. Use raw `apy_display` decimal in formulas
...
4. Present both scenarios clearly with calculated amounts. `delivery_timest` formatted as `yyyy-MM-dd` in UTC+0
...
4. Show: `invest_currency`/`exercise_currency` (coin pair), type, `invest_amount`, `exercise_price` (target price), APY (`apy_display × 100`%), delivery date **(UTC)** using `delivery_time`
5. Show total locked amount from balance
6. `delivery_time` formatted as `yyyy-MM-dd` in UTC+0. Do NOT display `settlement_price` for ongoing orders
...
7. Display: `settlement_currency`, `settlement_amount`, `settlement_price`, Realized APY (`apy_settlement × 100`%), delivery date **(UTC)** using `delivery_time` formatted as `yyyy-MM-dd`. All timestamps in UTC+0
8. `apy_display`, `apy_settlement` are decimals — multiply by 100 before appending `%`
...
**Expected Behavior**:
1. No API call — reply: "Order placement is not supported yet. Please visit the Gate official website or App to subscribe."
...
3. Display: `settlement_currency`, `settlement_amount`, Realized APY (`apy_settlement × 100`%), delivery date **(UTC)** using `delivery_time` formatted as `yyyy-MM-dd`, and note that both scenarios include principal + interest
4. All timestamps (`delivery_time`, `create_time`, `complete_time`) formatted as `yyyy-MM-dd` in UTC+0
5. `apy_display`, `apy_settlement` are decimals — multiply by 100 before appending `%`
