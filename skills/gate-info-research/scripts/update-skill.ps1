...
# Upstream: public https://github.com/gate/gate-skills (branch master). Raw
# version check uses raw.githubusercontent.com (no token for the default public repo).
$RemoteBranch  = 'master'
$RepoGit       = 'https://github.com/gate/gate-skills.git'
$ArchiveRoot   = 'gate-skills-master'   # top folder inside GitHub archive ZIP/tar
...
function Build-RemoteSkillMdUrl([string]$Name) {
  "https://raw.githubusercontent.com/gate/gate-skills/$RemoteBranch/skills/$Name/SKILL.md"
}
...
function Invoke-Apply {
  ...
  if (Get-Command git -ErrorAction SilentlyContinue) {
    ...
    try {
      ...
      & git clone --depth 1 --branch $RemoteBranch $RepoGit $cloneDir 2>$null
  ...
  $srcRoot = Join-Path $extractRoot $ArchiveRoot
  ...
  $srcTar = Join-Path $extractTar $ArchiveRoot
...
function Invoke-Check {
  ...
  $url = Build-RemoteSkillMdUrl $Name
  Log-Info 'fetching remote SKILL.md frontmatter …'
  # Public GitHub raw: no token. For private mirrors: $env:GATE_SKILL_GITLAB_TOKEN, $env:GATE_SKILL_HTTP_AUTH
  $authHeaders = @{}
  if ($env:GATE_SKILL_GITLAB_TOKEN) { $authHeaders['PRIVATE-TOKEN'] = $env:GATE_SKILL_GITLAB_TOKEN }
  elseif ($env:GATE_SKILL_HTTP_AUTH) {
    $parts = $env:GATE_SKILL_HTTP_AUTH -split ':', 2
    if ($parts.Count -eq 2) { $authHeaders[$parts[0].Trim()] = $parts[1].Trim() }
  }
  try {
    if ($authHeaders.Count -gt 0) {
      $resp = Invoke-WebRequest -Uri $url -UseBasicParsing -Headers $authHeaders
    } else {
