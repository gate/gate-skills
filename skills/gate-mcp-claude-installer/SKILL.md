---
name: gate-mcp-claudeinstaller
description: 当用户要一键安装 Claude Code (Claude CLI) 的 Gate MCP 和全部 Gate Skills 时使用。可安装 Gate MCP（main/dex/info/news 可选）以及 gate-skills 仓库中的全部 skills；默认不选时安装所有 MCP + 全部 skills，skills 始终全部安装不可选。
---

# Gate 一键安装（Claude Code：MCP + Skills）

用户说「一键安装 Gate」「安装 Gate MCP 和 skills」「Claude 安装 gate-mcp」等且在使用 **Claude Code（Claude CLI）** 时，使用本技能。
...
- 用户级配置：`~/.claude.json`（Windows：`%USERPROFILE%\.claude.json`）。若为目录形式则使用 `~/.claude/` 下对应配置。
- 若已存在则**合并**到现有 `mcpServers`，不覆盖其他 MCP。
- 配置说明：
  - **Gate (main)**：stdio，`command: npx`, `args: ["-y", "gate-mcp"]`
  - **Gate-Dex**：http，`url` + `headers["x-api-key"]` 固定为 MCP_AK_8W2N7Q
  - **Gate-Info / Gate-News**：http，`url`

### 3. 安装 gate-skills（全部）

- 从 https://github.com/gate/gate-skills 拉取 **skills/** 下所有子目录，复制到 `~/.claude/skills/`（或当前环境对应目录）。
- 使用脚本时加 `--no-skills` 可仅安装 MCP、不安装 skills。

### 4. 完成后提示

- 告知用户已安装的 MCP 列表和「已安装 gate-skills 全部 skills」（若未使用 --no-skills）。
- 提示重新打开 Claude Code 或新开会话以加载 MCP。
...
- DEX 的 x-api-key 已固定为 `MCP_AK_8W2N7Q`，写入配置。

从 GitHub 下载本 skill 后，在仓库根目录执行：  
`bash skills/gate-mcp-claudeinstaller/scripts/install.sh`  
或（仅安装 MCP）：  
`bash skills/gate-mcp-claudeinstaller/scripts/install.sh --no-skills`
