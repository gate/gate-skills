...
# Upstream: public https://github.com/gate/gate-skills (branch master). Version
# check uses https://raw.githubusercontent.com/... (anonymous; no token for the
# default public repo). Optional GATE_SKILL_GITLAB_TOKEN / GATE_SKILL_HTTP_AUTH
# support private mirrors only.
REMOTE_BRANCH="master"
REPO="https://github.com/gate/gate-skills.git"
# GitHub archive unpacks a top-level folder named gate-skills-master/
...
ARCHIVE_ROOT="gate-skills-master"
FETCH_REMOTE_ERR=""

# Raw URL for skills/<name>/SKILL.md on $REMOTE_BRANCH.
build_remote_skill_md_url() {
  local name="$1"
  printf 'https://raw.githubusercontent.com/gate/gate-skills/%s/skills/%s/SKILL.md\n' \
    "$REMOTE_BRANCH" "$name"
}
...
fetch_remote_skill_head() {
  ...
  # Public GitHub raw: no token. For private mirrors, optional auth:
  #   GATE_SKILL_GITLAB_TOKEN, GATE_SKILL_HTTP_AUTH
  local _auth_args=()
  if [ -n "${GATE_SKILL_GITLAB_TOKEN:-}" ]; then
    _auth_args=(-H "PRIVATE-TOKEN: ${GATE_SKILL_GITLAB_TOKEN}")
  elif [ -n "${GATE_SKILL_HTTP_AUTH:-}" ]; then
    _auth_args=(-H "${GATE_SKILL_HTTP_AUTH}")
  fi

  if command -v curl >/dev/null 2>&1; then
    log_dim "fetching via curl …"
    # Bash 3.x + set -u safe: only expand the array when it has elements.
    if [ "${#_auth_args[@]}" -gt 0 ]; then
      err=$(curl -sfL "${_auth_args[@]}" "$url" -o "$tmp" 2>&1) || err_rc=$?
    else
      err=$(curl -sfL "$url" -o "$tmp" 2>&1) || err_rc=$?
    fi
    if [ "${err_rc:-0}" -ne 0 ]; then
...
' ' ' | sed 's/[[:space:]]\+/ /g; s/^ //; s/ $//')
  ...
  if command -v wget >/dev/null 2>&1; then
    ...
    local _wget_hdr=()
    if [ -n "${GATE_SKILL_GITLAB_TOKEN:-}" ]; then
      _wget_hdr=(--header "PRIVATE-TOKEN: ${GATE_SKILL_GITLAB_TOKEN}")
    elif [ -n "${GATE_SKILL_HTTP_AUTH:-}" ]; then
      _wget_hdr=(--header "${GATE_SKILL_HTTP_AUTH}")
    fi
    if [ "${#_wget_hdr[@]}" -gt 0 ]; then
      err=$(wget -qO "$tmp" "${_wget_hdr[@]}" "$url" 2>&1) || err_rc=$?
    else
      err=$(wget -qO "$tmp" "$url" 2>&1) || err_rc=$?
    fi
    if [ "${err_rc:-0}" -ne 0 ]; then
      reason=$(printf '%s' "$err" | tr '
' ' ' | sed 's/[[:space:]]\+/ /g; s/^ //; s/ $//')
...
do_apply() {
  ...
  if command -v git >/dev/null 2>&1; then
    ...
    if git clone --depth 1 --branch "$REMOTE_BRANCH" "$REPO" "$CLONE_DIR" 2>/dev/null; then ok=1
    else
      log_warn "first clone attempt failed, retrying …"
      rm -rf "$CLONE_DIR" 2>/dev/null || true
      git clone --depth 1 --branch "$REMOTE_BRANCH" "$REPO" "$CLONE_DIR" 2>/dev/null && ok=1 || true
    fi
  else
...
cmd_check() {
  ...
  local remote_url
  remote_url=$(build_remote_skill_md_url "$NAME")
