...
**Context**: **This scenario is disabled in `gate-exchange-trading-copilot`.** For standalone “flash quote→one base” requests (e.g. 500 USDT to ETH), this skill must **not** invoke **any** MCP tool and must **not** name or list exchange tools in the reply—only hand off.
...
## Scenario 7: Query-Only Positions and Balances (Merged View)

**Context**: User wants a consolidated view of futures, margin, and spot (**S5**, query-only).

**Prompt Examples**:
- "Show all my positions and unrealized PnL."
- "List my perp positions, margin accounts, and spot balances."

**Expected Behavior**:
1. Call `cex_fx_list_fx_positions`, `cex_margin_list_margin_accounts`, and `cex_spot_get_spot_accounts`; merge and summarize.
2. No write tools unless the user adds execution intent.
...
## Scenario 14: Cross-Margin Unwind (Sell Base and Repay)

**Context**: User closes cross-margin / unified leveraged exposure by selling base and repaying loan (**S3**).

**Prompt Examples**:
- "Close my cross-margin ETH position and repay the loan."
- "Sell ETH from cross margin and pay back USDT borrow."

**Expected Behavior**:
1. Call `cex_margin_list_cross_margin_loans` / `cex_unified_get_unified_accounts` and `cex_spot_get_spot_accounts` as needed to see balances and liabilities.
2. Action Draft: sell leg + repay leg; after Y, call `cex_spot_create_spot_order` (sell, margin account) then `cex_unified_create_unified_loan` or `cex_margin_create_uni_loan` for repay per API.
...
**Context**: **This scenario is disabled in `gate-exchange-trading-copilot`.** For a single plan that combines **futures position reduction** with a **flash swap / convert** leg (**S2** + **S4**), this skill must **not** invoke **any** MCP tool and must **not** name or list exchange tools in the reply—only hand off.
...
**Context**: User clears working orders on a pair then places a new limit (**S6** → **S1**).
...
2. Action Draft 1: cancel all on pair; after Y, `cex_spot_cancel_all_spot_orders`.
3. Action Draft 2: new limit; after Y, `cex_spot_create_spot_order`.
