# Simple Earn Flexible (Uni) — `gate-cli` Tools Reference

`gate-cli` tools only. No REST paths or methods; use this document for skill behavior and tool arguments.

**Scope**: Simple Earn **flexible** (Uni) only. For fixed-term, see `references/fixed-earn-mcp-tools.md`.

### User-facing output (flexible Uni)

When presenting flexible Uni results to the user, **strip or omit every time-display field** from `gate-cli` responses (e.g. `time`, `create_time`, `update_time`, `timestamp`, settlement instants, history row times). Tables and prose must not include date/time columns or “at HH:MM” style facts unless the user explicitly asks for timing. Chart tools: do not print time-indexed series; at most state the latest estimated APY (or decline to show a timeline).

---

## 1. `gate-cli cex earn uni rate`

**Auth**: No

Estimated annualized rate per flexible currency. Use this to enumerate currencies with APY and to pick the highest-APY currency. For **min_lend_amount**, **min_rate**, and lend limits, call **`gate-cli cex earn uni currency`** for each currency you care about (this tool does not return full lend constraints).
...
Array of objects (typical): `currency`, estimated rate / APY fields as returned by `gate-cli`. Sort by rate to implement "subscribe to highest APY", then call **`gate-cli cex earn uni currency`** on the chosen currency before subscribe.

---

## 2. `gate-cli cex earn uni currency`
...
**Pre-call**: When subscribing, call this (or list) to get `min_rate` and pass it into `gate-cli cex earn uni lend`; otherwise the lend may fail.

---

## 3. `gate-cli cex earn uni lend`
...
## 5. `gate-cli cex earn uni change`
...
## 6. `gate-cli cex earn uni records`
...
## 7. `gate-cli cex earn uni interest`
...
## 8. `gate-cli cex earn uni interest-records`
...
## 9. `gate-cli cex earn uni interest-status`
...
## 10. `gate-cli cex earn uni chart`
...
Array of `{ "currency": string, "est_rate": string }`. `est_rate` is estimated APY (not percentage). Use the currency with highest `est_rate` for “subscribe to top APY”; confirm amount and call `gate-cli cex earn uni lend` after user confirmation.

---

## Tool summary (flexible)

| `gate-cli` tool | Auth | Use in skill |
|----------|------|--------------|
| `gate-cli cex earn uni rate` | No | APY per currency; top APY + currency list |
| `gate-cli cex earn uni currency` | No | Single-currency min_rate / limits for subscribe |
| `gate-cli cex earn uni lend` | Yes | Subscribe (lend) or redeem |
| `gate-cli cex earn uni lends` | Yes | Positions (single or all) |
| `gate-cli cex earn uni change` | Yes | Change min rate |
| `gate-cli cex earn uni records` | Yes | Lend/redeem history |
| `gate-cli cex earn uni interest` | Yes | Single-currency cumulative interest |
| `gate-cli cex earn uni interest-records` | Yes | Interest records |
| `gate-cli cex earn uni interest-status` | Yes | Reinvest status |
| `gate-cli cex earn uni chart` | No | APY chart |
