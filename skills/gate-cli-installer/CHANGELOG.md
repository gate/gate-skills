...
- **SKILL.md** / **README.md**: documented **`gate-cli`** binary resolution order (PATH → **`~/.local/bin`** → **`~/.openclaw/skills/bin`**), aligned with **`gate-runtime-rules.md`** §4 and current **`setup.sh`** install path (**`~/.local/bin`**).

## [2026.4.22-1] - 2026-04-22

### Changed

- **SKILL.md** refocused on **gate-cli** (via `setup.sh`) and **gate-skills** install; **removed** MCP-centric instructions and the **`references/mcp.md`** doc.
- **README.md** updated to match the gate-cli + skills quick start; auth errors → **`gate-cli config init`**.
