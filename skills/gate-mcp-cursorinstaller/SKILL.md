...
description: 当用户要一键安装 Gate 的 MCP 和 Skills 时使用。可安装 Gate MCP（main/wallet/info/news 可选）、以及 gate-skills 仓库中的全部 skills；默认不选时安装所有 MCP + 全部 skills，skills 始终全部安装不可选。
---
...
| MCP | Gate Wallet | https://api.gatemcp.ai/mcp/wallet，x-api-key 固定 |
...
1. **默认**：用户未指定选哪几个 MCP 时，安装**全部 MCP**（main、wallet、info、news）+ **全部 gate-skills**。
2. **可选 MCP**：用户可指定只安装部分 MCP（如只装 main、只装 wallet 等），按用户选择执行。
...
- 若用户未说明选哪些 MCP → 安装全部：main、wallet、info、news。
...
- 配置说明：
  ...
  - **Gate-Wallet**：`url` + `transport: streamable-http` + `headers["x-api-key"]` 固定为 MCP_AK_8W2N7Q
...
- 用法：  
  `./scripts/install.sh [--mcp main|wallet|info|news] ... [--no-skills]`  
  不传 `--mcp` 时安装全部 MCP；传多个 `--mcp` 则只安装指定项；`--no-skills` 仅安装 MCP。
- Wallet 的 x-api-key 已固定为 `MCP_AK_8W2N7Q`，写入 mcp.json。
