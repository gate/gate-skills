...
version: "2026.4.10-1"
updated: "2026-04-10"
description: "MCP execution specification for welfare eligibility, newcomer task list query, task claim, completion guidance, and newcomer reward claim."
---
...
|--------|---------|----------------|
| `0` | Unclaimed | Can be claimed via `cex_welfare_claim_task` |
| `1` | Claimed / in progress | Guide completion only |
| `2` | Completed, reward claimable | Can be claimed via `cex_welfare_claim_reward` |
...
Required order:
1. `cex_welfare_get_user_identity`
2. If `code=0`, `cex_welfare_get_beginner_task_list`
...
Required order:
1. `cex_welfare_get_user_identity`
2. `cex_welfare_get_beginner_task_list`
3. Select exactly one task with `status=0`
4. `cex_welfare_claim_task`
...
- Never call `cex_welfare_claim_task` for tasks that are already `1/2/3/4/5`.
...
4. Call `cex_welfare_claim_reward` sequentially for each selected task
