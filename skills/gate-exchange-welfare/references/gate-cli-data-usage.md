# Proper `gate-cli` Data Usage Examples
...
```javascript
const identity = await `gate-cli cex welfare identity`;
...
```javascript
const taskList = await `gate-cli cex welfare beginner-tasks`;
...
| Display Content | `gate-cli` Field | Notes |
|----------------|-----------|-------|
...
✅ Use the real `gate-cli` task list and real claim responses.
...
1. Uses the latest four-tool welfare `gate-cli` surface
