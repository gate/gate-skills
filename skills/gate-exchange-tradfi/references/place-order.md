...
Place a new TradFi order. **Before placing an order, call `gate-cli cex tradfi market symbol`** with the symbol to get the trading pair configuration; if the symbol is not found, do not place the order. **`gate-cli cex tradfi order create` supports setting take-profit and stop-loss price** (optional, per MCP). Use only MCP-documented tool names and parameters.

## MCP tools and parameters

| Tool | Purpose | Parameters (declare per MCP) |
| -----| --------| -----------------------------|
| `gate-cli cex tradfi market symbol` | Get symbol config (required before place order) | Pass symbol (trading pair). Returns config including `leverages`, `min_order_volume`, `step_order_volume`. If no result or error, symbol may not exist — do not place order. |
| `gate-cli cex tradfi order create` | Place new order | Required/optional from MCP. Typical: symbol, side, size (volume), price (limit) or market flag. **Optional: take-profit price, stop-loss price**, leverage. Do not pass parameters not documented in the MCP. |

### Symbol config validation (from `gate-cli cex tradfi market symbol`)

- **Pre-order requirement**: Before calling `gate-cli cex tradfi order create`, call **`gate-cli cex tradfi market symbol`** with the symbol. If the query returns no data or an error, **do not place the order** — the symbol may not exist; inform the user.
...
3. **Required before place order**: Call **`gate-cli cex tradfi market symbol`** with the symbol (trading pair). If the call returns no data or an error, **do not place the order** — tell the user the symbol may not exist.
...
7. After confirmation, call `gate-cli cex tradfi order create` with the confirmed parameters.
...
**Important**: The `id` and `log_id` returned by `gate-cli cex tradfi order create` **must not be used** as the order id for the cancel (`gate-cli cex tradfi order cancel`) or amend (`gate-cli cex tradfi order update`) APIs. For cancel or amend, the order id **must** come from **`gate-cli cex tradfi order list`** (the order list query). If the user wants to cancel or amend an order they just created, call `gate-cli cex tradfi order list` first and use the order id from that response.
...
**Expected Behavior**:
1. Extract symbol, side, size, price. Call `gate-cli cex tradfi market symbol`; if no result, do not place order and inform user.
...
4. Call `gate-cli cex tradfi order create` with only MCP-documented parameters.
...
**Expected Behavior**:
1. Extract symbol, side, size. Call `gate-cli cex tradfi market symbol`; if no result, do not place order.
...
3. Output parameters for user confirmation. Wait for confirmation.
4. Call `gate-cli cex tradfi order create`.
...
**Context**: User wants to place an order and set take-profit and/or stop-loss price at creation. **`gate-cli cex tradfi order create` supports this.**
...
**Expected Behavior**:
1. Extract symbol, side, size, price (or market), take-profit/stop-loss if given. Call `gate-cli cex tradfi market symbol`; if no result, do not place order.
...
4. After confirmation, call `gate-cli cex tradfi order create` with the parameters.
5. Respond with parameters used and result.
