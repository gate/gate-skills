...
name: gate-dex-x402
description: >
  Gate Wallet x402 payment module. Handles HTTP 402 Payment Required responses
  using the wallet. Use when the user wants to pay for a resource that returns
  402, call an API with x402 payment, or fetch a URL that requires payment.
  Supports four payment schemes: EVM exact (EIP-3009), EVM upto (Permit2),
  Solana exact (SPL), and Solana upto (SPL). Auto-signs and retries with
  PAYMENT-SIGNATURE.
metadata:
  version: "2.1.0"
  updated: "2026-03-23"
