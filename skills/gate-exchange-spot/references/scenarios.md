...
## V. Trigger Orders and TP/SL Automation (32-36)

## Scenario 32: Place Conditional Buy Trigger Order
**Context**: User wants a trigger order that buys BTC after a 5% drop from current price.
**Prompt Examples**:
...
3. Uses stale/non-BTC ticker data to compute the 5% trigger threshold.

## Scenario 33: Place Dual TP/SL Trigger Orders
**Context**: User wants ETH position protection with one take-profit and one stop-loss trigger order.
**Prompt Examples**:
...
3. Executes both legs without presenting two-order draft and confirmation checkpoint.

## Scenario 34: Query Single Trigger Order Progress
**Context**: User wants to know how far current market price is from trigger activation for one trigger/TP-SL order.
**Prompt Examples**:
...
3. Output `Trigger Progress Report` with current price, trigger price/rule, and remaining distance.
   **Unexpected Behavior**:
1. Uses execution price instead of trigger price when computing trigger distance.
2. Reports progress using outdated ticker snapshot.
3. Returns vague text ("almost there") without numeric gap details.

## Scenario 35: Batch Cancel BTC Buy Trigger Orders
**Context**: User wants all BTC buy-side trigger orders to be found and canceled in one operation.
**Prompt Examples**:
...
3. Omits canceled-vs-not-found breakdown in final report.

## Scenario 36: Cancel Single Trigger/TP-SL Order by ID
**Context**: User asks whether a specific trigger order is still active and wants it canceled if active.
**Prompt Examples**:
...
3. Returns "canceled" without tool result evidence or updated order status.
