...
For every scenario that includes `create_spot_order`, the assistant must ask for explicit final user confirmation immediately before placing the order.  
...
- Tools: `get_spot_accounts` → `create_spot_order`
...
- Tools: `get_currency` → `get_currency_pair` → `get_spot_tickers`
...
- Tools: `get_spot_accounts` → `get_spot_tickers`
...
- Tools: `cancel_all_spot_orders` → `get_spot_accounts`
...
- Tools: `get_spot_tickers` → `create_spot_order`
...
- Tools: `create_spot_order` → `create_spot_order`
...
- Tools: `get_wallet_fee` → `get_spot_tickers`
...
- Tools: `list_spot_orders` (status=`open`) → `amend_spot_order`
...
- Tools: `list_spot_my_trades` → `get_spot_accounts`
...
- Tools: `list_spot_orders` (status=`open`) → `cancel_spot_order` → `get_spot_accounts`
...
- Tools: `list_spot_my_trades` → `get_spot_accounts` → `create_spot_order`
...
- Tools: `list_spot_my_trades` → `get_spot_tickers` → `create_spot_order`
...
- Tools: `get_spot_accounts` → `get_spot_tickers` → `create_spot_order` (sell) → `create_spot_order` (buy)
...
- Tools: `get_spot_tickers` → `create_spot_order` → `get_spot_accounts`
...
- Tools: `get_spot_candlesticks` → `create_spot_order`
...
- Tools: `get_spot_order_book` → `create_spot_order`
