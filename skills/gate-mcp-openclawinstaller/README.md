...
- Installs a complete Gate MCP server set for OpenClaw / mcporter.
- Supports full install (default) or interactive selection (`--select`).
- Uses mcporter auth flows for OAuth2-enabled endpoints when required.
...
# or
./skills/gate-mcp-openclawinstaller/scripts/install.sh -s
```
...
- MCP fragments: `scripts/mcp-fragments/openclaw/`
